				...
				function WhaleLayer() {
					...
					React.useEffect(() => {
						...
						function stepBubbles(c, dt) {
							...
							// A wider stream than v1: at the default 45% opacity the old
							// 1.2–4.6px, 1px-stroke circles were nearly invisible.
							if (bubbles.length < 40 && Math.random() < dt * 9) {
								bubbles.push({
						...
						function step(c, dt, t) {
							...
							for (let i = 0; i < fish.length; i++) {
								...
								const target = cruise * (1 + 1.8 * f.panic)
								f.speed += (target - f.speed) * Math.min(1, dt * 2.2)
						...
						function paint(c) {
							...
							if (bubbles.length > 0) {
								...
								const body = light ? '#0ea5e9' : '#bae6fd'
								const rim = light ? '#0c4a6e' : '#e0f2fe'
								g.lineWidth = 1.2
								g.fillStyle = body
								g.strokeStyle = rim
								for (const b of bubbles) {
									/* translucent body */
									g.globalAlpha = bubbleAlpha * 0.35
									g.beginPath()
									g.arc(b.x, b.y, b.r, 0, Math.PI * 2)
									g.fill()
									/* rim */
									g.globalAlpha = bubbleAlpha * 0.8
									g.beginPath()
									g.arc(b.x, b.y, b.r, 0, Math.PI * 2)
									g.stroke()
									/* highlight on the bigger ones */
									if (b.r > 2.2) {
										g.globalAlpha = bubbleAlpha
										g.beginPath()
										g.arc(b.x - b.r * 0.32, b.y - b.r * 0.32, b.r * 0.3, 0, Math.PI * 2)
										g.stroke()
									}
								}
							}
