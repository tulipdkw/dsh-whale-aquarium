...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			'.dwa-footGlyph{font-size:15px;line-height:1}',
			...
			apply(ctx) {
				...
				function WhaleLayer() {
					...
					React.useEffect(() => {
						...
						function stepBubbles(c, dt) {
							...
							if (bubbles.length < 34 && Math.random() < dt * 7) {
								bubbles.push({
						...
						function paint(c) {
							...
							if (bubbles.length > 0) {
								g.lineWidth = 1
								g.strokeStyle = light ? '#0369a1' : '#bae6fd'
								for (const b of bubbles) {
									g.globalAlpha = c.opacity * 0.5
									g.beginPath()
				...
				function FootAction(props) {
					...
					return React.createElement(
						...
						React.createElement('span', { className: 'dwa-footGlyph' }, '🐋'),
